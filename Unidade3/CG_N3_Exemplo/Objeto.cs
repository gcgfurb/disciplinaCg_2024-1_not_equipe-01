...
namespace gcgcg
{
  internal class Objeto  //TODO: deveria ser uma class abstract ..??
  {
    ...
    public List<Ponto4D> pontosLista = [];
    ...
    public void ObjetoAtualizar()
    {
      ...
      if (objetosLista.Count > 0) // Atualiza BBox dos filhos
      {
        objetosLista[0].bBox.Atualizar(matriz, objetosLista[0].pontosLista);
      }
    }
    ...
    public void FilhoRemover(Objeto filho)
    {
      ...
      {
        removido = this.objetosLista.Remove(filho);
        if (!removido)
        {
          objetosLista[0].FilhoRemover(filho);
        }
      }
    }
    ...
    public Objeto GrafocenaBuscaPoligonosMundo(char _rotuloInicial)
    {
      foreach (var objeto in objetosLista)
      {
        if (objeto.PontosListaTamanho == 0)
          continue;

        var charProximo = _rotuloInicial;

        while (objeto.rotulo >= charProximo)
        {
          if (objeto.rotulo == charProximo)
            return objeto;

          charProximo = Utilitario.CharProximo(charProximo);
        }
      }
