...
namespace gcgcg
{
    public class Mundo : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            GL.ClearColor(0.51f, 0.5f, 0.49f, 1.0f);
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (input.IsKeyPressed(Keys.Q)) // Move pra esquerda
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarPe(-0.05);
                    }
                    else if (input.IsKeyPressed(Keys.W)) // Move pra direita
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarPe(0.05);
                    }
                    else if (input.IsKeyPressed(Keys.A)) // Diminuir tamanho do raio
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarRaio(-0.05);
                    }
                    else if (input.IsKeyPressed(Keys.S)) // Aumentar tamanho do raio
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarRaio(0.05);
                    }
                    else if (input.IsKeyPressed(Keys.Z)) // Diminuir ângulo
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarAngulo(-1);
                    }
                    else if (input.IsKeyPressed(Keys.X)) // Aumentar ângulo
                    {
                        SrPalito srPalitoAtual = (SrPalito)objetoSelecionado;
                        srPalitoAtual.AtualizarAngulo(1);
                    }
